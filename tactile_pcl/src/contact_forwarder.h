...
public:
	...
	void connectInput(F& f)
	{
		...
		incoming_connection_ =
		    f.registerCallback(message_filters::SimpleFilter<tactile_msgs::TactileContacts>::EventCallback(
		        boost::bind(&ContactForwarder::cb, this, _1)));
	}
...
private:
	void cb(const ros::MessageEvent<const tactile_msgs::TactileContacts>& evt)
	{
		...
		for (tactile_msgs::TactileContacts::_contacts_type::const_iterator it = msg->contacts.begin(),
		                                                                   end = msg->contacts.end();
		     it != end; ++it) {
